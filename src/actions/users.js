...
export const fetchWaitingUsers = () => async (dispatch) => {
  const response = await axios.get("users/?is_refused=true").catch((err) => {
    if (navigator.onLine === false)
      return dispatch({
        type: USERS_WAITING_LOAD_FAILURE,
        error: "Check your internet connection",
      });
    return dispatch({
      type: USERS_WAITING_LOAD_FAILURE,
      error: "Users: " + err.response.detail,
    });
  });
  if (response.status === 200)
    return dispatch({
      type: USERS_WAITING_LOAD_SUCCESS,
...
export const PatchUserApproval = (userId, is_refused) => async (dispatch) => {
  const response = await axios
    .patch(`users/${userId}/is_refused/`, { is_refused })
    .catch((err) => {
      if (navigator.onLine === false)
        return dispatch({
          type: USERS_WAITING_PATCH_FAILURE,
          error: "Check your internet connection",
        });
      return dispatch({
        type: USERS_WAITING_PATCH_FAILURE,
        error: "User Approval: " + err.response.detail,
      });
    });
  if (response.status === 201 || response.status === 200)
    return dispatch({
      type: USERS_WAITING_PATCH_SUCCESS,
      payload: userId,
...
export const fetchUsers = (
  ...
  const response = await axios
    .get(
      `users/?is_refused=${is_refused}&is_active=${is_active}&first_name=${first_name}&last_name=${last_name}&email=${email}`
    )
    .catch((err) => {
      if (navigator.onLine === false)
        return dispatch({
...
export const DisableEnableUser = (userId, is_active) => async (dispatch) => {
  const response = await axios
    .patch(`users/${userId}/is_active/`, { is_active })
    .catch((err) => {
      if (navigator.onLine === false)
        return dispatch({
